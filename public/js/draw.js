	...
	var  mousemove = function(e){
		...
		if(graphType == 'square'){
			if(canDraw){
				context_bak.beginPath();
				clearContext();
				var topLeft_x = Math.min(startX, x);
				var topLeft_y = Math.min(startY, y);
				var width = Math.abs(startX - x);
				var height = Math.abs(startY - y);
				context_bak.rect(topLeft_x, topLeft_y, width, height);
				// context_bak.stroke();
				context_bak.fillStyle = "white";
				context_bak.fill();
			}
		//直线
		...
		//橡皮擦 不管有没有在画都出现小方块 按下鼠标 开始清空区域
			...
			if(canDraw){
				...
				context2.clearRect(x - size * 10 ,  y - size * 10 , size * 20 , size * 20);
			}
		}
	};
