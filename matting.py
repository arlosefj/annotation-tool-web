...
def closed_form_matting_with_binary(image, binary):
    image = cv2.GaussianBlur(image, (5,5) ,0)
    # cv2.imshow('blur', image)
    kernel_size = 3
    times = 10
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size)) 
    erosion = cv2.erode(binary, kernel, iterations = times)
    dilation = cv2.dilate(binary, kernel, iterations = times)
    h, w = binary.shape
    trimap = np.zeros(binary.shape, dtype = np.uint8)
    for i in range(h):
        for j in range(w):
            if (erosion[i][j] != dilation[i][j]):
                trimap[i][j] = 127
            else: 
                if (dilation[i][j] == 0):
                    trimap[i][j] = 0
    ...
    alpha = closed_form_matting_with_trimap(image/255.0, trimap/255.0)
    return (alpha * 255.0).astype('uint8')
...
def closed_form_matting_with_trimap(image, trimap, trimap_confidence=100.0):
    ...
    return closed_form_matting_with_prior(image, 
        trimap, 
...
def main():
    ...
    if args.solve_fg:
        ...
        output = alpha
    cv2.imshow('output', (output * 255.0))
...
if __name__ == "__main__":
    ...
    cv2.imwrite('./data/image-'+localtime+'.png', image)
    cv2.imwrite('./data/mask-'+localtime+'.png', mask)

    binary = np.where((mask>127), 255, 0).astype('uint8')
    alpha = closed_form_matting_with_binary(image, binary)
    cv2.imwrite('./img/retMask.png', alpha)

    binary = np.where((alpha>127), 255, 0).astype('uint8')
    cv2.imwrite('./data/retMask-'+localtime+'.png', binary)